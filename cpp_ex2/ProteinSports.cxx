...
std::string ProteinSports::test () {
	return("testing test\n"); 
}
std::string ProteinSports::a2aaa(std::string a) {
	auto res = std::lower_bound(avec.begin(),avec.end(),a);
	int index = std::distance(avec.begin(),res); // res-avec.begin()
	return(aaavec[index]); 	
}	

std::string ProteinSports::aaa2a(std::string aaa) {
	int i = 0;
	std::string a = "";
	for (auto ai : aaavec) {
		if (ai == aaa) {
...
float ProteinSports::getWeight(std::string aa) {
	float weight = 0;
	int index = 0;
	if (aa.length() == 1) {
...
void ProteinSports::display(std::vector<int> x){
...
void ProteinSports::menu(int n_question = 20) {
