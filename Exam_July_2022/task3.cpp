...
public:
    ...
    std::map< std::string, int > ObsoleteStats(std::string filename); 
private:
    int x =0;
};


std::map< std::string, int > Parser::ObsoleteStats(std::string filename) {
    std::cout << "this is obs stats "<<std::endl;
    ...
    while (std::getline(infile, line)) {
        ...
        if (std::regex_search(line,stop)) {
            if (match && is_obs && con) {
				...
				count_consider = count_consider +  v1.size();
                v1.clear();
                match = false;
                is_obs = false;
                con = false;
    ...
    res["consider"] = count_consider ;
    res["non_consider"] = count_non_con;
...
std::vector<std::vector<std::string> > Parser::ConsiderTable_Namespace(std::string filename, std::string namesp) {
    ...
    std::regex rgoid("^id: ");
    std::regex rnmsp("^namespace: "+ namesp);
    std::regex robs("^is_obsolete: ");
    std::regex rcon("^consider: ");
    std::regex stop("^$");
        ...
        if (argc == 3) {
         ...
         //    std::cout << "this is considerTable cdm"  <<  std::endl;
            ...
            if (cmd == "obsoleteStats") {
                ...
                auto res = ps.ObsoleteStats(argv[1]) ;
